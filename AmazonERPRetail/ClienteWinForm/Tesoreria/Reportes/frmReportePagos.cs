...
{
    public partial class frmReportePagos : FrmMantenimientoBase
    {
        int TotalRow = 0;
        ...
        private void btBuscar_Click(object sender, EventArgs e)
        {
            ...
            oListaDetalle = AgenteCtasPorPagar.Proxy.ListarPagosPorPeriodo(4, txtDesde.Value, txtHasta.Value);
        ...
        private void dgvListado_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (oListaDetalle != null)
            {
                ...
                {   
                    // COLUMNA
                    if (e.ColumnIndex == dgvListado.Columns["impTotalBase"].Index)
                    {
                        if (OrdenarColumnas01)
                        {
                            oListaDetalle = (from x in oListaDetalle orderby x.impTotalBase ascending select x).ToList();
                            OrdenarColumnas01 = false;
                        }
                        else
                        {
                            oListaDetalle = (from x in oListaDetalle orderby x.impTotalBase descending select x).ToList();
                            OrdenarColumnas01 = true;
                        }
                    }

                    // COLUMNA
                    if (e.ColumnIndex == dgvListado.Columns["impTotalSecun"].Index)
                    {
                        if (OrdenarColumnas03)
                        {
                            oListaDetalle = (from x in oListaDetalle orderby x.impTotalSecun ascending select x).ToList();
                            OrdenarColumnas03 = false;
                        }
                        else
                        {
                            oListaDetalle = (from x in oListaDetalle orderby x.impTotalSecun descending select x).ToList();
            ...
            using (ExcelPackage oExcel = new ExcelPackage(newFile))
            {
                ...
                if (oHoja != null)
                {
                    ...
                    foreach (ProvisionesE item in oListaDetalle)
                    {
                        ...
                        oHoja.Cells[InicioLinea, 3].Value = item.CodPartidaPresu;
                        oHoja.Cells[InicioLinea, 4].Value = item.desPartidaPresu;
                        ...
                        oHoja.Cells[InicioLinea, 8].Value = item.desDocumento;
                        oHoja.Cells[InicioLinea, 9].Value = item.NumSerie;
                        oHoja.Cells[InicioLinea, 10].Value = item.NumDocumento;
                        oHoja.Cells[InicioLinea, 11].Value = item.TipCambio;
                        oHoja.Cells[InicioLinea, 12].Value = item.CodMonedaProvision;

                        oHoja.Cells[InicioLinea, 13].Value = item.impTotalBase;
                        oHoja.Cells[InicioLinea, 14].Value = item.impTotalSecun;

                        oHoja.Cells[InicioLinea, 15].Value = item.EstadoProvision;
                        oHoja.Cells[InicioLinea, 16].Value = item.idComprobante;

                        oHoja.Cells[InicioLinea, 17].Value = item.numFile;
                        oHoja.Cells[InicioLinea, 18].Value = item.numVoucher;
                        oHoja.Cells[InicioLinea, 19].Value = item.codCuenta;
        ...
        private void btnExportar_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                TituloGeneral = "Pagos " + dia + "_" + mes + "_" + anio;
