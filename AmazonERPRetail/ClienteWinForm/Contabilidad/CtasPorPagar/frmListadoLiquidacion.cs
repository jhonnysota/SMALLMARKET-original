...
{
    public partial class frmListadoLiquidacion : FrmMantenimientoBase
    {
        ...
        public frmListadoLiquidacion()
        {
            ...
            InitializeComponent();
            FormatoGrid(dgvFondosFijosRend, true);
        }
        ...
        List<LiquidacionE> ListaLiquidacion = null;
        String RutaPdf = String.Empty;
        String RutaImagen = String.Empty;
        String RutaGeneral = String.Empty;
        //List<FondoFijoE> oListaFondoFijo = null; //Para saber si tienen asignada alguna caja o rendición...
        List<UsuarioFondoFijoE> oListaUsuarioFondos = null; //Permisos a las planillas...
        ...
        public override void Nuevo()
        {
            try
            {
                if (oListaUsuarioFondos != null && oListaUsuarioFondos.Count > 0)
                {
                    if (oFondoSeg != null)
                    {
                        if (!oFondoSeg.Asignacion)
                        {
                            if (!oFondoSeg.Edicion)
                            {
                                Global.MensajeComunicacion("No tiene permiso para agregar registros.");
                                return;
                            }
                        }
                    }
                ...
                Form oFrm = Application.OpenForms.Cast<Form>().FirstOrDefault(x => x is frmLiquidacion);
                ...
                oFrm = new frmLiquidacion((rbFijos.Checked == true ? "102" : "168"), oFondoSeg)
                {
                    MdiParent = MdiParent
                };
        ...
        public override void Buscar()
        {
            try
            {
                ...
                Boolean est1 = false;
                Boolean est2 = false;
                Boolean BuscarDcmto = false;
                String TipoFondo = "102";
                String td = "%";
                String ser = "%";
                String num = "";
                ...
                if (rbUnoDoc.Checked)
                {
                    ...
                    {
                        td = cboDocumentos.SelectedValue.ToString();
                    }

                    if (!String.IsNullOrWhiteSpace(txtSerie.Text.Trim()))
                    {
                        ser = txtSerie.Text.Trim();
                    }
                ...
                if (oListaUsuarioFondos != null && oListaUsuarioFondos.Count > 0)
                {
                    oFondoSeg = oListaUsuarioFondos.Find
                    (
                        delegate (UsuarioFondoFijoE f) { return f.TipoFondo == Convert.ToInt32(TipoFondo); }
                    );

                    if (oFondoSeg != null)
                    {
                        if (oFondoSeg.Asignacion) //Visualiza lo perteneciente al usuario
                        {
                            bsLiquidacion.DataSource = ListaLiquidacion = AgenteCtasPorPagar.Proxy.ListarLiquidacion(idEmpresa, idLocal, VariablesLocales.SesionUsuario.IdPersona, dtpFecIni.Value.Date, dtpFecFin.Value.Date, est1, est2, TipoFondo, BuscarDcmto, td, ser, num);
                        }
                        else
                        {
                            if (oFondoSeg.Visualizar) //Visualiza de todos los usuarios
                            {
                                bsLiquidacion.DataSource = ListaLiquidacion = AgenteCtasPorPagar.Proxy.ListarLiquidacion(idEmpresa, idLocal, 0, dtpFecIni.Value.Date, dtpFecFin.Value.Date, est1, est2, TipoFondo, BuscarDcmto, td, ser, num);
                            }
                            else
                            {
                                bsLiquidacion.DataSource = null;
                            }
                        }
                    }
                    else
        ...
        public override void Editar()
        {
            try
            {
                if (((LiquidacionE)bsLiquidacion.Current).desEstado != "CERRADO")
                {
                    if (oFondoSeg != null)
                    {
                        if (!oFondoSeg.Asignacion)
                        {
                            if (!oFondoSeg.Visualizar)
                            {
                                Global.MensajeComunicacion("No tiene permiso para visualizar el registro.");
                                return;
                            }
                            else
                            {
                                if (oFondoSeg.Visualizar && !oFondoSeg.Edicion)
                                {
                                    Global.MensajeComunicacion("Solo podrá visualizar el registro.");
                                }
                            }
                        }
                    } 
                ...
                if (bsLiquidacion.Count > 0)
                {
                    ...
                    oFrm = new frmLiquidacion((LiquidacionE)bsLiquidacion.Current, oFondoSeg)
                    {
                        MdiParent = MdiParent
                    };
        ...
        public override void Anular()
        {
            try
            {
                if (oFondoSeg != null)
                {
                    if (!oFondoSeg.Edicion)
                    {
                        Global.MensajeComunicacion("No tiene permiso para eliminar registros.");
                        return;
                    }
                }
                ...
                {
                    OrdenPagoE oOrdenPago = AgenteTesoreria.Proxy.ObtenerOrdenPago(((LiquidacionE)bsLiquidacion.Current).idOrdenPago);

                    if (oOrdenPago != null)
                    {
                        Global.MensajeFault("La liquidación no se puede eliminar porque esta asociada con una O.P.");
                        return;
                    }

                    if (Global.MensajeConfirmacion(Mensajes.AvisoEliminacion) == DialogResult.Yes)
                    {
                        AgenteCtasPorPagar.Proxy.EliminarLiquidacion(((LiquidacionE)bsLiquidacion.Current).idEmpresa, ((LiquidacionE)bsLiquidacion.Current).idLocal, ((LiquidacionE)bsLiquidacion.Current).idLiquidacion);
                        Buscar();
        ...
        public override void Imprimir()
        {
            try
            {
                if (bsLiquidacion.Count > 0)
                {
                    ...
                    frmImpresionBase oFrm = new frmImpresionBase(oLiquidacion, "Vista Previa de la Liquidación");
        ...
        {
            frmLiquidacion oFrm = sender as frmLiquidacion;
        ...
        private void frmListadoLiquidacion_Load(object sender, EventArgs e)
        {
            ...
            //oListaFondoFijo = AgenteTesoreria.Proxy.ListarFondoFijoPorResponsable(VariablesLocales.SesionUsuario.Empresa.IdEmpresa, VariablesLocales.SesionLocal.IdLocal, VariablesLocales.SesionUsuario.IdPersona);
            oListaUsuarioFondos = AgenteSeguridad.Proxy.ListarUsuarioFondoFijo(VariablesLocales.SesionUsuario.Empresa.IdEmpresa, VariablesLocales.SesionUsuario.IdPersona);
            Buscar();
        }

        private void dgvFondosFijosRend_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                Editar();
        ...
        private void tsmiGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                LiquidacionE current = (LiquidacionE)bsLiquidacion.Current;
                
                if (current != null)
                {
                    //Para obtener el estado correcto
                    current = AgenteCtasPorPagar.Proxy.ObtenerLiquidacion(current.idEmpresa, current.idLocal, current.idLiquidacion);
                    ...
                    if (!current.Estado) //0 ó false=Abierto 1 ó true=Cerrado
                    {
                        ...
                        AgenteCtasPorPagar.Proxy.CerrarLiquidacion(current, VariablesLocales.SesionUsuario.Credencial, TipoFondo, current.Tipo168);
                        Buscar();
        ...
        private void dgvFondosFijosRend_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                if ((Boolean)dgvFondosFijosRend.Rows[e.RowIndex].Cells["Estado"].Value == true)
                {
                    if (e.Value != null)
                    {
        ...
        {
            if (bsLiquidacion.Current != null)
            {
                lblTitulo.Text = "Registros " + bsLiquidacion.Count.ToString();
            }
        }

        private void tsmiCorreo_Click(object sender, EventArgs e)
        {
            try
            {
                BuscarImagen();
                ...
                {
                    CrearPdf(oLiquidacion);
                    frmEnvioCorreos oFrm = new frmEnvioCorreos(oLiquidacion, RutaPdf);
                    oFrm.ShowDialog();
                }
            }
            catch (Exception ex)
        ...
        private void tsmVoucher_Click(object sender, EventArgs e)
        {
            try
            {
                if (!String.IsNullOrWhiteSpace(((LiquidacionE)bsLiquidacion.Current).numVoucher) && !String.IsNullOrWhiteSpace(((LiquidacionE)bsLiquidacion.Current).AnioPeriodo))
                {
                    ...
                    {
                        AnioPeriodo = ((LiquidacionE)bsLiquidacion.Current).AnioPeriodo,
                        numVoucher = ((LiquidacionE)bsLiquidacion.Current).numVoucher,
                        idComprobante = ((LiquidacionE)bsLiquidacion.Current).idComprobante,
                        numFile = ((LiquidacionE)bsLiquidacion.Current).numFile,
                        MesPeriodo = ((LiquidacionE)bsLiquidacion.Current).MesPeriodo,
        ...
        private void tsmiAbrir_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (current != null)
                {
                    if (current.Estado) //0 ó false=Abierto  1 ó true=Cerrado
                    {
                        if (current.idOrdenPago > 0)
                        {
                            OrdenPagoE pagoE = AgenteTesoreria.Proxy.ObtenerOrdenPago(current.idOrdenPago);

                            if (pagoE != null)
                            {
                                if (pagoE.indEstado == "C" || pagoE.indEstado == "A")
                                {
                                    Global.MensajeAdvertencia("La orden de pago se encuentra cerrada. No podrá abrir la liquidación.");
                                    return;
                                }
                            }
                        }

                        AgenteCtasPorPagar.Proxy.AbrirLiquidacion((LiquidacionE)bsLiquidacion.Current, VariablesLocales.SesionUsuario.Credencial);
                        Buscar();
        ...
        private void tsmiLimpiar_Click(object sender, EventArgs e)
        {
            try
            {
                if (!((LiquidacionE)bsLiquidacion.Current).Estado)
                {
                    AgenteCtasPorPagar.Proxy.LimpiarVoucherLiquidacion(((LiquidacionE)bsLiquidacion.Current).idLiquidacion, VariablesLocales.SesionUsuario.Credencial);
                    Buscar();
