...
{
    public partial class frmImprimirEEFFHorizontal : Form
    {

        public frmImprimirEEFFHorizontal()
        {
            InitializeComponent();
            ImagenRuta();
        }

        public frmImprimirEEFFHorizontal(List<EEFFRatiosE> oLista_)
          :this()
        {
            oListaRatios = oLista_;
        }

        public frmImprimirEEFFHorizontal(List<ReporteEEFFItemE> oLista,String Anio)
          :this()
        {
            oListaEEFF = oLista;
            Anio_ = Anio;
        ...
        void CrearReporte()
        {
            if (oListaRatios != null && oListaRatios.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(RutaGeneral.Trim()))
                {
                    ...
                    PaginaEEFFHorizontal ev = new PaginaEEFFHorizontal();
                    ...
                    for (int i = 0; i < oListaEEFF.Count; i++)
                    {
                        ...
                        if (oListaEEFF[i].TipoTabla == "TIT" && oListaEEFF[i].desItem == "ACTIVO")
                        {
                            ...
                            cell = new PdfPCell(new Paragraph(Anio_+'/'+AnioAnterior, FontFactory.GetFont("Arial", 7f, iTextSharp.text.Font.BOLD))) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT };
                            TablaCabDetalle.AddCell(cell);

                            cell = new PdfPCell(new Paragraph(" ", FontFactory.GetFont("Arial", 7f, iTextSharp.text.Font.BOLD))) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT };
                            TablaCabDetalle.AddCell(cell);
                        ...
                        cell = new PdfPCell(new Paragraph(oListaEEFF[i].AnalisisH, FontFactory.GetFont("Arial", 7f))) { Border = 0, HorizontalAlignment = Element.ALIGN_RIGHT };
        ...
        private void frmImprimirEEFFHorizontal_Load(object sender, EventArgs e)
        {
            try
            {
...
class PaginaEEFFHorizontal : PdfPageEventHelper
{
    ...
    public override void OnStartPage(PdfWriter writer, Document document)
    {
        ...
        TituloGeneral = "           ANALISIS HORIZONTAL";
