...
{
    public partial class frmGuiaGenesis : FrmMantenimientoBase
    {

        public frmGuiaGenesis(Int32 idEmpresa, Int32 idLocal, String idDocumento, String Serie, String Numero)
        {
        ...
        void LlenarDatos()
        {
            if (DocumentoPrevio != null)
            {
                ...
                if (DocumentoPrevio.idDocumento == EnumTipoDocumentoVenta.GV.ToString())
                {
                    ...
                    List<String> oLista = new List<String>(DocumentoPrevio.Glosa.Split('/'));
                    ...
                    if (oLista != null && oLista.Count > 0)
                    {
                        ...
                        lblGlosa.Text = "Atención: " + Cadena.ToString().Trim();
                    }
                    else
                    {
                ...
                foreach (EmisionDocumentoDetE item in DocumentoPrevio.ListaItemsDocumento)
                {
                    ...
                    {
                        item.nomArticulo = (!UsarNombreCompuesto ? item.nomArticulo : (item.tipArticulo == "AR" ? item.desNomArtCompuesto : item.nomArticulo)) + "(" + item.LoteProveedor + ")";
                    }
                    else
                    {
                ...
                if (oFrm.ShowDialog() == DialogResult.OK && oFrm.oImpresora != null)
                {
                    ...
                    else
                    {
                        if (oFrm.oImpresora.EsMatricial)
                        {
                            ...
                            {
                                ImpresionManager.RecuperarUtilImpresion("20519167434").ImprimirGuiaRemision(DocumentoPrevio, NombreImpresora, (int)EnumTipoImpresionGuiaRemision.VENTA);
                            }
                        }
                        else
                        {
        ...
        private void frmGuiaGenesis_Load(object sender, EventArgs e)
        {
            Grid = false;
