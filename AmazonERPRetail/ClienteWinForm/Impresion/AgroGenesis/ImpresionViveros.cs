...
{
    public class ImpresionViveros : IImpresion
    {
        ...
        {
            GuiasViveros oGuiaImpresion = new GuiasViveros();
        ...
        private void Facturas(EmisionDocumentoE oFactura, String nomImpresora)
        {
            ...
            FacturasViveros oFacturaImpresion = new FacturasViveros();
            String NombreArticulo = String.Empty;
            ...
            //Cabecera (Dato, x, y)
            oFacturaImpresion.AgregarDatos(oFactura.RazonSocial, "33", "65.5");
            oFacturaImpresion.AgregarDatos(oFactura.Direccion, "32", "73.5");
            oFacturaImpresion.AgregarDatos(oFactura.numRuc, "32", "82.5");
            //oFacturaImpresion.AgregarDatos(oFactura.fecEmision.ToString("d"), "162", "82.5"); //Por revisar

            if (oFactura.ListaCanjeGuias != null && oFactura.ListaCanjeGuias.Count > 0)
            {
                oFacturaImpresion.AgregarDatos(oFactura.ListaCanjeGuias[0].numSerieGuia + "-" + oFactura.ListaCanjeGuias[0].numDocumentoGuia, "45", "90"); //guia
            }
            else
            {
                oFacturaImpresion.AgregarDatos(" ", "45", "90"); //guia
            }
            ...
            foreach (EmisionDocumentoDetE item in oFactura.ListaItemsDocumento)
            {
                ...
                {
                    NombreArticulo = (!UsarNombreCompuesto ? item.nomArticulo : (item.tipArticulo == "AR" ? item.desNomArtCompuesto : item.nomArticulo));
                    subtot = oFacturaImpresion.AlinearDerecha(Convert.ToDecimal(item.subTotal).ToString("N2").Length, 10) + Convert.ToDecimal(item.subTotal).ToString("N2");
                    ...
                    oFacturaImpresion.AgregarItems(Convert.ToInt32(item.Cantidad).ToString("N0") + "|"
                        + NombreArticulo + "|" + oMoneda.desAbreviatura + Precio + "|"
                        + (item.indCalculo == true ? oMoneda.desAbreviatura + subtot : (oFactura.DsctoGlobal > 0 ? item.SubTotalCad : "")), ControlDocumento.cantCaracteres);
                }
                else
                {
            ...
            {
                oFacturaImpresion.AgregarDatos(oMoneda.desAbreviatura + "  " + oFactura.totTotal.ToString("N2"), "178", "247");
                ////Total en Letras
        ...
        private void BoletaVenta(EmisionDocumentoE oBoleta, String nomImpresora)
        {
            ...
            BoletasViveros oBoletaImpresion = new BoletasViveros();
            String NombreArticulo = String.Empty;
            MonedasE oMoneda = (from x in VariablesLocales.ListaMonedas
            ...
            foreach (EmisionDocumentoDetE item in oBoleta.ListaItemsDocumento)
            {
                ...
                {
                    NombreArticulo = (!UsarNombreCompuesto ? item.nomArticulo : (item.tipArticulo == "AR" ? item.desNomArtCompuesto : item.nomArticulo));
                    subtot = oBoletaImpresion.AlinearDerecha(Convert.ToDecimal(item.subTotal).ToString("N2").Length, 10) + Convert.ToDecimal(item.subTotal).ToString("N2");
        ...
        {
            NotasDeCreditoViveros oNcImpresion = new NotasDeCreditoViveros();
        ...
        {
            NotasDeDebitoViveros oNdImpresion = new NotasDeDebitoViveros();
            String NombreDocumento = String.Empty;
            String Moneda = String.Empty;
            String Ruc = String.Empty;
            MonedasE oMoneda = (from x in VariablesLocales.ListaMonedas
