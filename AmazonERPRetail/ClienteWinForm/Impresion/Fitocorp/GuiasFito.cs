...
namespace ClienteWinForm.Impresion.Fitocorp
{
    public class GuiasFito
    {
        ...
        public void AgregarItems(String ItemTexto)
        {
            ...
            Items[ContadorItems, 2] = "80"; // Eje y
            ContadorItems++;
            ...
            Items[ContadorItems, 1] = "65";// Eje x
            Items[ContadorItems, 2] = "80"; // Eje y
            ContadorItems++;

            // Lote
            Items[ContadorItems, 0] = itemsDatos[3];
            Items[ContadorItems, 1] = "73";// Eje x
            Items[ContadorItems, 2] = "80"; // Eje y
            ContadorItems++;

            // Descripción del Articulo
            //Comprobando si en la descripción del articulo existe enter...
            bool EncontroEnter = (itemsDatos[4].IndexOf("\r\n") > 0);

            // Comprobar si es mayor a la cantidad de digitos permitido de lo contrario pasar a la siguiente linea...
            if (EncontroEnter)
            {
                DivideItemDetEnter(itemsDatos[4].Length, DigitosArt, itemsDatos[4]);
                ContadorItems++;
            }
            else
            {
                // Comprobar si es mayor a la cantidad de digitos permitido de lo contrario pasar a la siguiente linea...
                DivideItemDet(itemsDatos[4].Length, DigitosArt, itemsDatos[4]);
                // Continuo almacenando
                ContadorItems++;
            }

            // Cantidad
            Items[ContadorItems, 0] = AlinearDerecha(itemsDatos[5].Length, DigitosCant) + itemsDatos[5];
            Items[ContadorItems, 1] = "176"; // Eje x
            Items[ContadorItems, 2] = "80"; // Eje y
            ContadorItems++;
            ...
            if (lenghtDet > maxCharDet)
            {
                ...
                while (i < SplitDetalle.Length)
                {
                    ...
                    if (ContadorLineas < 1)
                    {
                        ...
                        Items[ContadorItems, 1] = "98"; // Eje x
                        Items[ContadorItems, 2] = "80"; // Eje y
                        ContadorLineas++;
                        Linea = String.Empty;
                    }
