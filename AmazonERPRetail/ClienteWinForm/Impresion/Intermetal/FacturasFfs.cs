...
namespace ClienteWinForm.Impresion.Intermetal
{
    public class FacturasFfs
    {
        ...
        public void AgregarItems(String ItemTexto)
        {
            ...
            Items[ContadorItems, 1] = "0"; // Eje x
            Items[ContadorItems, 2] = "113"; // Eje y
            ContadorItems++;

            // Cantidad
            Items[ContadorItems, 0] = AlinearDerecha(itemsDatos[1].Length, DigitosCant) + itemsDatos[1];
            Items[ContadorItems, 1] = "28"; // Eje x
            Items[ContadorItems, 2] = "113"; // Eje y
            ContadorItems++;
            
            // Unidad de Medida
            Items[ContadorItems, 0] = itemsDatos[2];
            Items[ContadorItems, 1] = "47"; // Eje x
            Items[ContadorItems, 2] = "113"; // Eje y
            ContadorItems++;

            // Precio Unitario
            Items[ContadorItems, 0] = AlinearDerecha(itemsDatos[4].Length, DigitosPu) + itemsDatos[4];
            Items[ContadorItems, 1] = "140"; // Eje x
            Items[ContadorItems, 2] = "113"; // Eje y
            ContadorItems++;

            // Total
            Items[ContadorItems, 0] = AlinearDerecha(itemsDatos[5].Length, DigitosImporte) + itemsDatos[5];
            Items[ContadorItems, 1] = "162";// Eje x
            Items[ContadorItems, 2] = "113"; // Eje y
            //ContadorItems++;

            // Descripción del Articulo
            //Comprobando si en la descripción del articulo existe enter...
            bool EncontroEnter = (itemsDatos[3].IndexOf("\r\n") > 0);

            // Comprobar si es mayor a la cantidad de digitos permitido de lo contrario pasar a la siguiente linea...
            if (EncontroEnter)
            {
                DivideItemDetEnter(itemsDatos[3].Length, DigitosArt, itemsDatos[3]);
                ContadorItems++; //Agregado para aprobar
            }
            else
            {
                DivideItemDet(itemsDatos[3].Length, DigitosArt, itemsDatos[3]);
            ...
            foreach (string item in listaTextoP)
            {
                if (item.Length < maxCharDet)
                {
                    if (ContadorLineas < 1)
                    {
                        ...
                        Items[ContadorItems, 1] = "56";// Eje x
                        Items[ContadorItems, 2] = "113";// Eje y
                        ContadorLineas++;
                    }
                    else
                    {
                        ContadorItems++;
                        Items[ContadorItems, 0] = String.Empty; //Código
                        Items[ContadorItems, 1] = "56";// Eje x
                        Items[ContadorItems, 2] = "113";// Eje y
                        ...
                        ContadorItems++;
                        Items[ContadorItems, 0] = item; // Descripción
                        Items[ContadorItems, 1] = "56";// Eje x
                        Items[ContadorItems, 2] = "113";// Eje y
                ...
                else
                {
                    ...
                    DivideItemDet(item.Length, maxCharDet, item);
            ...
            if (lenghtDet > maxCharDet)
            {
                ...
                while (i < splitDetalle.Length)
                {
                    ...
                    else
                    {
                        ...
                        Items[ContadorItems, 1] = "56";// Eje x //Anterios medida 56 -113
                        Items[ContadorItems, 2] = "113";// Eje y
                        ContadorItems++;
                        Items[ContadorItems, 0] = String.Empty;
                        Items[ContadorItems, 1] = "56";// Eje x
